...
# ax1 = fig.add_subplot(111)
# spectrum = np.zeros(fnum)
# for p_i in np.arange(1.24e7, 1.24e7, 1):
#         for n_i in range(0,1,1):
#                 for m_i in range(0, 1, 1):
#                         for i_i in range(0, fnum):
#                                 spectrum[i_i] += lor(f_array[i_i], dw(p_i, m_i, n_i))
#
#
# ax1.plot(f_array, spectrum, linewidth=2, color=tableau20[0])
#
# #ax1.set_xlabel('$x$', fontsize=30)
# #ax1.set_ylabel('$\\frac{p^2(t)}{m\omega_0}$', fontsize=30)
# plt.tight_layout()
