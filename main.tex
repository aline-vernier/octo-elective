...
\end{equation}
Cette équation admet deux solutions (que l'on peut écrire sous forme exponentielle) $\lambda_+ = \exp^{i\phi}$ et  $\lambda_- = \exp^{-i\phi}$ : 
...
\end{equation}
On rappelle que $\Tr{T} = \lambda_{+}+\lambda_{-}$ et $\det{T}=\lambda_{+}\lambda_{-} = 1$, deux relations qui nous seront utiles par la suite.
Lorsque $\lambda_{+}$ et $\lambda_{-}$ sont distinctes, les vecteurs propres associés $u_{+},\,u_{-}$ forment une base, et on peut alors exprimer tout rayon $x = \left[r_n, r'_n\right]$ comme une combinaison linéaire de ces deux vecteurs propres $x = au_{+}+bu_{-}$. Après $n$ tours de cavité, $x_n = a(\lambda_+)^nu_{+}+b(\lambda_-)^nu_{-}$.
