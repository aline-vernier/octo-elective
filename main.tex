...
Lorsque les électrons sont déplacés de leur position d'équilibre par un champ électrique externe (tel qu'un champ laser) d'une quantité $\delta x$ (voir Figure~\ref{fig:osc_plas}), ils subissent une force de rappel $F=e E_x$ dûe au champ électrostatique qui résulte de ce déplacement. On peut alors très simplement écrire l'équation du mouvement en 1D de ces électrons~: 

\begin{equation}
    m_e\frac{d^2\delta x}{dt^2}=eE_x(\delta x)
\end{equation}

On applique alors le théorème de Gauss $\nabla\cdot\textbf{E}=-\frac{e n_0}{\epsilon_0}$~: 

\begin{equation}
    E_x(\delta x) = -en_0\frac{\delta x}{\epsilon_0}
\end{equation}

On obtient alors l'équation du mouvement des électrons~:
\begin{equation}
    \frac{d^2\delta x}{dt^2}+\frac{n_0e^2}{\epsilon_0 m_e}\delta x =0
\end{equation}
...
Soumis à une perturbation, les électrons suivent donc, dans le régime linéaire, un mouvement oscillant collectif $\delta x(t) = \delta x_0\cos(\omega_p t +\theta_0)$. Il est intéressant de représenter ce mouvement dans ce que l'on appelle l'espace des phases qui est tout simplement une représentation graphique de la quantité de mouvement $p$ en fonction de la postition. Pour une oscillation harmonique dans le régime non-relativiste, on a les trajectoires représentées figure~\ref{fig:phase_space}.

\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=8cm]{}
\end{center}
\caption{Mouvement oscillant harmonique dans l'espace des phases}
...
\label{fig:sillage}
\end{figure}
\subsubsection{Trajectoire des électrons dans l'onde plasma}
...
Le premier terme représente l'énergie de masse de l'électron et son énergie cinétique, et le deuxième terme, son énergie potentielle. Afin de simplifier les notations, on normalise cet Hamiltonien en utilisant les variables $u_z=\frac{p_z}{m_ec}$ et $\phi = \frac{e\Phi}{m_ec^2}$. On obtient alors~:
