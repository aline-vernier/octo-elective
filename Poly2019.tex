...
\begin{center}
\includegraphics[width=15cm]{pictures/SpherMirr.png}
\end{center}
...
On peut facilement montrer que les rayons fortment hors-axe ne sont pas focalisés au même point que les rayons paraxiaux par une optique sphérique. Cette propriété géométrique est ce qu'on appelle l'aberration sphérique. Il existe un grand nombre d'aberrations, les plus fréquentes dans les optiques sphériques étant l'astigmatistme (qui peut affecter les yeux !) donnant lieu à l'existence de deux foyers distincts pour deux directions orthogonales perpendiculaires à l'axe optique et l'aberration sphérique. La seule surface permettant une focalisation parfaitement stigmatique quel que soit l'écart à l'axe optique d'un rayon parallèle à ce dernier est la parabole (c'est de ça dont il s'agit quand on parle d'optiques asphérisées, comme les lentilles des téléphones portables ou les lentilles de collimation des diodes laser).
...
\label{fig:astig}
\begin{center}
\includegraphics[width=11cm]{pictures/Astig.png}
\end{center}
\caption{Astigmatisme : il peut provenir soit de l'existence de deux longueurs focales différentes dans les deux directions, soit d'un angle important entre l'axe optique et l'axe de la propagation de la lumière. Dans ce dernier cas, on note $T$ le foyer tangentiel et $S$ le foyer sagittal.}
\end{figure}
...
\label{fig:astig}
\begin{center}
\includegraphics[width=11cm]{pictures/abSpher.png}
\end{center}
\caption{Aberration sphérique : cette aberration géométrique apparaît lorsqu'on utilise une optique sphérique à grande ouverture.}
\end{figure}

On remarque aussi que la formule dite "des opticiens" \ref{eq:lensmaker} dépend de l'indice de réfraction qui est une fonction de la longueur d'onde (voir par exemple https://refractiveindex.info/). Cela implique que la focale dépend de la longueur d'onde, donnant lieu à ce qu'on appelle l'aberration chromatique. Afin d'y remédier, on peut souvent utiliser des miroirs (achromatiques dans le sens des aberrations, mais dont le coefficient de réflectivité peut en revanche très fortement dépendre de la longueur d'onde) ou des optiques spéciales combinant plusieurs verres pour compenser la variation d'indice avec la longueur d'onde. C'est le cas notamment des objectifs d'appareil photo, conçus pour travailler dans tout le spectre visible, ou d'un grand nombre d'objectifs de microscope. 

\section{Description matricielle d'éléments optiques}

Un ensemble de systèmes optiques travaillant dans les conditions de Gauss (== optique paraxiale) constitue une application linéaire, on peut donc exprimer les trajectoires des rayons par produits de \textit{matrices de transfert}, aussi appelées matrices de Gauss. Ce formalisme est  très pratique pour calculer numériquement la propagation d'un grand ensemble de rayons, notamment lors de la conception de cavités laser (voir section~\ref{sec:MTCavite} et Chapitre~\ref{ch:resPass} ).



\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=13cm]{pictures/ABCD.png}
\end{center}
\caption{Schéma de principe de la représentation en matrices de transfert}
\label{fig:matrix}
\end{figure}

Un rayon est complètement caractérisé dans un plan orthogonal à l'axe par sa distance $h(z)$ à l'axe optique et l'angle définissant sa direction de propagation $\theta(z)$. La convention de signe pour les angles est la même qu'en trigonométrie : le sens positif est défini dans le sens inverse des aiguilles d'une montre et l'origine au niveau de l'axe optique. 
L'approximation paraxiale nous permet d'assimiler $\mathrm{sin}(\theta)\sim\mathrm{tan}(\theta)\sim\theta$, et on a ainsi une relation linéraire entre $\left(r_1, \theta_1\right)$ et $\left(r_1, \theta_2\right)$, positions et angles avant et après passage dans le système optique. Si on définit $r_1'$ et $r_2'$ tels que $\theta_1 \sim (dr/dz)_{z_1} = r_1'$ et $\theta_2 \sim (dr/dz)_{z_2} = r_2'$. Du fait de la linéarité du système optique il existe des coefficients $A, B, C$ et $D$ tels que~: 
...
\begin{gather}
 ...
 =
  \begin{bmatrix}
   ...
   C & D 
   \end{bmatrix}
   \begin{bmatrix} r_1 \\ r'_1 \end{bmatrix}
\end{gather}
...
\noindent\fcolorbox{black}{myGray}{%
    \minipage[t]{\dimexpr1\linewidth-2\fboxsep-2\fboxrule\relax}
        \subsection{\'Elements de base (travaux dirigés)}\label{subsec:el_base}
...
Une cavité est un système optique dans lequel la lumière effectue des allers-retours et parcourt ainsi un grand nombre de fois le même ensemble d'éléments optiques, le formalisme des matrices de transfert est donc approprié à l'étude de la stabilité des cavités. Nous allons voir dans cette partie comment l'utiliser.
...
Après $n$ tours de cavité, un rayon de coordonnées $\left[r, r'\right]$ dans le plan $\Pi$ peut soit rester au voisinage de l'axe soit s'en éloigner. Une cavité pour laquelle les rayons restent confinés est dite \textbf{stable}. Lorsque les rayons divergent en-dehors de la cavité, cette cavité est \textbf{instable}. 
...
Si le résonateur est stable, pour tout rayon initial de coordonnées $\left[r_0, r'_0\right]$, le rayon $\left[r_n, r'_n\right]$ ne doit pas diverger lorsque $n$ augmente. Cela implique que la matrice~:
\begin{gather}
  \begin{bmatrix}
   A & B \\
   C & D 
...
Il existe plusieurs manières de déterminer une condition sur les éléments de la matrice $T$ d'une cavité stable. Ce document vous en propose deux, la première plus mathématique et la seconde peut-être moins élégante mais restant plus proche de l'interprétation physique. 

\subsubsection{Démonstration n°1}
...
La quantité $\sin n\theta$ contient un terme \textit{exponentiellement croissant} avec $n$ si $|b|>0$, et la matrice $T^n$ diverge alors. Par conséquent, pour que le résonanteur soit stable, $\theta$ doit être réel, et d'après l'équation \ref{eq:thetaib}, cela implique~:
...
\subsubsection{Démonstration n°2}
On peut aussi démontrer cette condition par diagonalisation de la matrice $T$. 
En utilisant le fait que le déterminant de la matrice vaut 1 sur un tour (milieux d'arrivée et de départ identique), la résolution de $\det\left[T-\lambda I\right] = 0$ amène à l'équation aux valeurs propres suivante~:
\begin{equation}
...
Après $n$ tours de cavité, $x_n = a(\lambda_+)^nu_{+}+b(\lambda_-)^nu_{-}$. Ce rayon reste confiné dans la cavité si $|\lambda_+|\leq1$ et $|\lambda_-|\leq1$. Comme $\lambda_{+}\lambda_{-} = 1$, alors $|\lambda_+|=|\lambda_-|=1$. 
...
Maintenant que nous avons établi une relation générale permettant d'évaluer si la cavité que l'on cherche à concevoir sera stable ou pas, nous allons étudier le cas particulier d'une cavité linéaire.
...
\end{center}
\caption{Diagramme de stabilité. Où se situent les cavités stables ? Situez la cavité confocale, la cavité concentrique et le Fabry-Pérot plan-plan. Où se situent les cavités symétriques ? (Schéma à compléter à la main par vos soins !)}
\label{fig:diag_stab}
\end{figure}
...
où $q_0$ est un nombre complexe. En définissant $x_0 = 0$ et $y_0 = 0$ puis on réécrit la répartition transverse d'amplitude sous la forme~: 
...
Nous pouvons à présent donner une interprétation physique à ces équations. 
\begin{itemize}
...
Ces relations démontrent que la position du waist image dépend non seulement de la position du waist objet mais aussi de sa taille. Lorsque $\sigma>>z_R$, l'onde au niveau du foyer objet est quasi-sphérique (celui-ci est dans le champ lointain du faisceau). Le front d'onde au niveau de la lentille s'apparente donc au front d'onde d'un point source, on obtient alors : $\sigma\sigma'=ff'$. On retrouve ici les relations de Newton de l'optique géométrique.
...
\begin{equation}
    w_0'=\frac{1}{w_0}\frac{f\lambda}{\pi}
\end{equation}
...
Le terme $\phi_0$ représente le déphasage par propagation d'une onde plane et le terme $\phi_g$ est la phase de Gouy. En prenant pour origine le waist du faisceau, on peut calculer $\phi_g$ correspondant à une propagation libre~:
\begin{equation}
...
\noindent\fcolorbox{black}{myGray}{%
    \minipage[t]{\dimexpr1\linewidth-2\fboxsep-2\fboxrule\relax}
        \textbf{Calculs de la taille et de la position du waist en cavité :\\}
            Soit une cavité linéaire de longueur $L$ et dont les miroirs sont de courbures $R_1$ et $R_2$ telle que sur la Figure~\ref{fig:cell}
          \\
          \\
          \begin{itemize}
...
Ces résonateurs sont dits ouverts, à la différence des résonateurs micro-ondes qui sont fermés et confinent ainsi le champ dans les trois dimensions. Bien que les résonateurs ouverts présentent des pertes transverses du fait de la diffraction du champ, il existe dans les résonateurs ouverts des modes comportant de très faibles pertes. On définit de tels modes comme une configuration électromagnétique que l'on écrit~:
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=13cm]{pictures/Concentric.png}
\end{center}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=15cm]{pictures/Confocal.png}
\end{center}
...
\section{Modes propres transverses et valeurs propres }
...
Si on écrit l'amplitude du champ dans le cas décrit Fig.~\ref{fig:cell} après un tour complet de cavité \cite{svelto}, on obtient~:

\begin{equation}
    \tilde E(x,y,2L)=\exp(-2\iu kL)\iint K(x,y; x_1,y_1) \tilde E(x_1, y_1, 0)dx_1dy_1
    \label{eq:gen_field}
\end{equation}
...
\begin{equation}
     \tilde E(x,y,2L)=\tilde\sigma\exp(-2\iu kL)\tilde E(x,y,0)
\end{equation}
...
\begin{equation}
    \tilde\sigma \tilde E(x,y,0)=\iint K(x,y; x_1,y_1) \tilde E(x_1, y_1, 0)dx_1dy_1
    \label{eq:fredholm}
\end{equation}

qui est une équation intégrale de Fredholm du second type admettant des solutions $ \tilde E_{lm}(x,y,0)$ \textbf{auto-similaires après chaque période en cavité}. Les valeurs propres des solution $ \tilde E_{lm}(x,y,0)$ définies par un couple d'entiers ${l,m}$ seront notées $ \tilde\sigma_{lm}$.
Comme nous l'avons souligné précédemment, le module de ces valeurs propres représente les pertes par diffraction, et il est par conséquent inférieur à 1. Le déphasage total acquis, $\Delta\phi_{lm}=-2kL+\phi_{lm}$ doit être un multiple de $2\pi$ afin que l'interférence soit constructive c'est-à-dire $\phi_{lm}=-2\pi n$. On déduit alors facilement~: 
...
\noindent\fcolorbox{black}{myGray}{%
    \minipage[t]{\dimexpr1\linewidth-2\fboxsep-2\fboxrule\relax}
        ...
        \begin{equation*}
             \tilde E(x,y,2L)=\exp(-2\iu kL)\iint K(x,y; x_1,y_1)~E(x_1, y_1, 0)dx_1dy_1
        \end{equation*}
...
Nous allons prendre ici l'exemple d'une cavité de longueur $L$ ayant deux miroirs de réflectivités en puissance $R_1$ et $R_2$. Soient $P_0$ la puissance initiale, et $T_i$ le coefficient de pertes fractionnaires \textit{par passage} dûes à la diffraction et autres pertes intra-cavité. La puissance $P(t_1)$ au temps $t_1=2L/c$, c'est-à-dire après un tour complet de la cavité s'exprime alors~: 
...
Nous ne ferons ici la démonstration que pour le cas d'une symétrie rectangulaire.

Nous allons chercher des solutions à une équation de propagation paraxiale, c'est-à-dire des solutions de la forme $E(x,y,z,t)=u(x,y,z)\exp^{-\iu kz}\exp^{\iu \omega t}$ telles que les variations de $u$ soient faibles lorsque $z$ varie. Rappelez-vous que nous avons établi que la distribution transverse du champ en cavité doit être auto-similaire, il ne peut donc y avoir de variation rapide de l'enveloppe du champ au cours de la propagation. En partant de l'équation d'onde pour une onde scalaire~:
...
Dans cette formule, on reconnaît la forme d'une onde gaussienne dont le profil transverse est modifié par une fonction $g$ ne dépendant que de $x$ et une fonction $h$ ne dépendant que de $y$. Ce choix est justifié dans le cadre des cavités laser par leur géométrie typique qui présente en général des symétries selon les plans $Oxz$ et $Oyz$.
...
Cette équation est celle d'un oscillateur harmonique . Afin que $g(X)$ conserve une valeur finie, ila condition $a/2=2m$, avec $m$ entier naturel doit être respectée. Les solutions de ces équations sont connues, il s'agit des polynômes d'Hermite~:
...
L'onde gaussienne correspond à l'ordre zéro (m=n=0) et les ondes d'ordres supérieurs satisfont à la loi ABCD, ce qui peut se montrer en injectant l'expression générale $u_{mn}$ dans l'intégrale de propagation (un exercice assez calculatoire et pas forcément édifiant).
...
Le champ éléctrique décrit par l'équation~\ref{eq:HermiteGauss} donne d'une part une dépendance spatiale de l'amplitude du champ en fonction de $m$ et $n$, mais aussi une dépendance du déphasage acquis par propagation. Les fréquences de résonance des modes transverses sont donc différentes dans le cas général et dépendent de $m$ et $n$. 
...
Chaque mode transverse, caractérisé par un couple d'indices $(m,n)$, peut osciller à une infinité de fréquences fixées par l'indice $p$. Pour chaque mode \textbf{transverse}, il existe une infinité de modes \textbf{longitudinaux} de fréquences caractérisées par l'indice $p$. L'écart en fréquence entre modes longitudinaux de la cavité vaut :
...
La Figure~\ref{fig:modes_long_et_trans} présente un exemple arbitraire de spectrographe donnant la répartition des modes en cavité. Dans un laser, il sera évidemment nécessaire de de tenir compte de la réponse spectrale du gain du milieu amplificateur pour obtenir le spectre d'émission. 


\chapter{Application : accélération d'électrons par laser}

\section{Introduction}
Le contenu de ce cours est un peu spécial puisqu'il présente une application des lasers qui n'est pas de l'optique, mais à la croisée des chemins entre l'optique, l'interaction laser-matière et l'électrodynamique relativiste. Nous survolerons par conséquent plusieurs notions de physique. Ce support donne les grandes lignes et les concepts essentiels qui vous permettront d'avoir une idée schématique des processus permettant l'accélération d'électrons par laser. 

Comme nous l'avons souligné dans l'introduction, l'essor de l'utilisation des lasers impulsionnels est remarquable : ils ont d'une part peuplé des domaines aussi éloignés que la découpe de tôle et la chirurgie de l'\oe{}il, et ils trouvent d'autre part des applications en recherche fondamentale. Ce succès a été récemment salué par le prix Nobel décerné en 2018 à G.~Mourou et D.~Strickland pour leur travail pionnier sur l'amplification par dérive de fréquence qui a permis de dépasser de plus d'un ordre de grandeur les intensités crêtes des lasers de pointe. Dans un grand nombre d'activités applicatives ou de recherche, c'est en effet l'intensité crête qui fait l'attractivité des lasers impulsionnels : cette dernière peut atteindre $10^{22}\,\mathrm{W/cm^2}$ sur les lignes de lumières les plus intenses. L'interaction entre un laser aussi intense et la matière permet d'accéder à des phénomènes physiques dits \textit{relativistes} car ils mettent en jeu des énergies cinétiques de l'ordre de l'énergie de masse des particules mises en mouvement par le champ. Pour un électron, cela correspond à une énergie cinétique de $m_ec^2 = 0.511\,\mathrm{MeV}$. 


Depuis la fin du XIX\textsuperscript{e} siècle, on accélère des particules en utilisant des champs électriques, d'abord statiques, puis radiofréquences. Afin d'obtenir des énergies cinétiques toujours plus importantes, les accélérateurs ont depuis atteint des dimensions colossales, ce qui en parallèle a stimulé la recherche de sources plus compactes. Le saut d'intensité crête des impulsions laser provoqué par l'utilisation de la technique d'amplification par dérive de fréquence a éveillé un intérêt dans ce sens et des particules (en particulier des électrons et des protons) ont pu être accélérées par interaction entre des impulsions laser et des solides ou des gaz. 
Nous appréhenderons ici les phénomènes fondamentaux permettant l'accélération d'électrons par interaction entre un laser impulsionnel ultra-intense et un gaz moléculaire. Nous discuterons tout d'abord des accélérateurs coventionnels et de leurs limitations, ainsi que des raisons pour lesquelles l'accélération par laser peut permettre de dépasser ces limites. Nous décrirons ensuite les phénomènes physiques qui ont lieu dans la matière lorsqu'on l'illumine à des intensités supérieures à $10^{16}\,\mathrm{W/cm^2}$. L'arrivée de l'impulsion provoque tout d'abord l'ionisation du milieu, et génère ainsi un plasma. Lorsque l'intensité dépasse les quelques $10^{18}\,\mathrm{W/cm^2}$, selon la densité éléctronique du plasma, on peut alors déclencher une excitation d'ondes plasma de forte amplitude. Lorsque l'onde plasma prend un caractère non-linéaire, le champ éléctrique qui en résulte peut dépasser $100\,\textrm{GV/m}$. Nous verrons alors comment ce champ électrique permet d'accélérer des électrons.
Nous reviendrons enfin aux technologies laser qui ont permis d'atteindre de si hautes intensités sur une des premières expériences de ce type. 

\section{Considérations sur les accélérateurs conventionnels}

Le principe de base de l'accélération de particules repose sur la génération d'un champ électrique accélérateur ainsi que sur l'injection d'électrons dans un tel champ. Le tout premier appareil conçu s'apparentant à un accélérateur de particules est le tube de Geissler (ou tube cathodique). Il s'agit d'un tube de verre, contenant une faible pression de gaz, souvent des gaz rares comme le néon ou l'argon, mais aussi d'autres gaz tels que le sodium ou le mercure. De part et d'autre de ce tube sont placées deux électrodes sur lesquelles on applique une haute tension. Le champ éléctrique ainsi créé provoque l'ionisation du milieu ainsi qu'un courant électrique. Une génération de lumière dont la couleur dépend du gaz utilisé dûe à la recombinaison des électrons avec les ions (et la désexcitation radiative qui s'ensuit) par fluorescence peut être observée. Lorsque le tube est sous vide poussé, on appelle ce type de dispositif un tube de Crookes (du scientifique anglais W.~Crookes qui est son inventeur). Dans un tube de Crookes, un rayonnement lumineux est aussi émis, mais seulement au niveau de l'anode. Ces rayonnements, à l'époque mystérieux furent nommés rayons cathodiques. En 1897, J.~J.~Thomson (le même que celui du modèle atomique de Thomson !) démontra que ces rayonnement provenaient d'une particule, alors inconnue, qui fut nommée ``électron''. 

C'est en partant de ce principe que Cockcroft and Walton développèrent le tout premier accélérateur de particules lors de leurs travaux avec Rutherford. Il s'agissait d'un accélérateur de protons destiné à l'étude de la structure interne de l'atome. L'accélérateur fut mis en service en mars 1932. Un mois plus tard, Rutherford, Cockcroft et Walton mirent en évidence la première fission atomique artificielle par bombardement de protons sur une cible de lithium. L'énergie des protons à l'époque était de 710$\,\mathrm{keV}$. Ce type d'accélérateur, que l'on appelle un accélérateur électrostatique est toujours très utilisé aujourd'hui, mais où les particules accélérées sont des électrons plutôt que des protons. L'application la plus commune en est sans doute les tubes à rayons X utilisés pour les radiographies médicales. 

L'énergie des particules issues d'un tel accélérateur est cependant limitée par le champ de claquage (même sous "vide" !) à environ 1$\,\mathrm{MV/m}$. On peut pour certaines applications utiliser des gaz isolants tels que le $\mathrm{SF}_6$ afin de multiplier cette limite par un facteur 30.

Une autre méthode, l'accélération radiofréquence fut inventée par Ising et développée par Wider{\o}e en 1928 (voir Fig.~\ref{fig:wideroe} et Fig.~\ref{fig:acc_dec}. En connectant une série d'électrodes de polarités alternées, on peut arriver à des énergies assez importantes au prix de dimensions colossales comme le SLAC (l'accélérateur linéaire de Stanford) qui peut accélérer des électrons à 50$\,\mathrm{GeV}$, sur une distance de 3,2$\,\mathrm{km}$ ! En général ce type d'accélérateur est utilisé autour de quelques dizaines de MeV. Ce dispositif ingénieux permet de démultiplier la tension d'alimentation étage après étage, tant que le faisceau (impulsionnel !) reste en phase avec le champ accélérateur. On peut faire un calcul simple des dimensions de l'accélérateur pour des énergies non-relativistes : le gain d'énergie après le $n$-ième étage est le suivant~:

\begin{equation}
    E_n=nqU_0\sin\left(\phi_s\right)
    \label{eq:linac_energy}
\end{equation}
où $n$ est le nombre d'étages d'accélération, $q$ est la charge de la particule, $U_0$ la tension d'alimentation (limitée par le champ de claquage dans le système, de l'ordre de 10 à 100$\,$MV/m) et $\phi_s$ la phase relative entre l'impulsion de particules et la tension sinusoïdale. 
...
Pour des vitesses relativistes, le gain d'énergie n'est évidemment plus linéairement croissant avec le nombre d'étages, et la longueur totale de l'accélérateur devient colossale (l'exemple du SLAC cité précédemment l'illustre). 
...
Pour plus d'informations sur les accélérateurs, l'article de Bernhard Holzer sur les accélérateurs et leur limites \cite{holzer} est une lecture tout-à-fait appropriée. 
...
Il est assez clair que l'un des facteurs limitants est la taille des structures accélératrices qui devient monumentale lorsqu'on atteint des vitesses relativistes. C'est la problèmatique que l'accélération laser-plasma pourrait permettre de résoudre. Additionnellement à cette perspective de compacification des accélérateurs il a été découvert que les paquets d'électrons générés par laser sont très courts (aussi courts que 1.5$\,$fs !) et parfaitement synchronisés avec l'impulsion laser. Ces caractéristiques très intéressantes ont aussi contribué à l'essor de la recherche de l'accélération de particules par laser. 

\section{L'accélération par interaction laser-plasma}

\subsection{Introduction}

"Puisque la limite des champs accélérateurs est le champ de claquage, utilisons donc un milieu déjà claqué !" Ainsi pourrions nous décrire avec un brin d'humour l'idée de base de l'utilisation des champs présents dans un plasma pour accélérer des électrons. \textit{[Cette dernière remonte d'après \cite{malka} à une intervention de Budker en 1956 \cite{budker} mais dont l'auteure du document présent n'a pu trouver de trace en ligne. L'article en question se trouve-t-il même dans les archives du CERN ?...]}  
...
En 1979, Dawson et Tajima proposèrent une idée portée par la technologie en emergence : l'onde plasma pouvait être excitée par laser \cite{tajima}, dans un plasma généré par l'impulsion laser elle-même. 
...
\begin{equation}
    V(z)=-\frac{Z^*e^2}{4\pi\epsilon_0z}-eE_0z
\end{equation}

La figure~\ref{fig:potentiel} montre que le champ extérieur abaisse la barrière de potentiel. Lorsque cette dernière passe en-dessous de l'énergie de liaison $V_i$, on a ionisation par suppression de barrière. Pour calculer ce champ, on calcule le maximum du potentiel (abscisses positives sur la figure~\ref{fig:potentiel}) au point $z_M$ tel que $\left(\partial V/\partial z\right)_{z_M}=0$. Ce maximum se situe en $z_M=\left(\frac{Z^*e}{4\pi\epsilon_0E}\right)^{1/2}$, où le potentiel vaut~:

\begin{equation}
    V(z_M)=2e\left(\frac{Z^*eE}{4\pi\epsilon_0}\right)^{1/2}
    \label{eq:max_potentiel}
\end{equation}

\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=15cm]{pictures/suppression_barriere.png}
\end{center}
\caption{Potentiel coulombien et effet du champ laser sur le potentiel. En bleu sombre : potentiel non-perturbé. En augmentant l'intensité du champ, on augmente la probabilité d'ionisation par effet tunnel (bleu ciel), jusqu'à la suppression de barrière. }
\label{fig:potentiel}
\end{figure}
...
Faisons tout d'abord un calcul d'ordre de grandeur pour aiguiser notre intuition. L'intensité est une densité surfacique de puissance. Estimons tout d'abord la puissance instantanée $P$ d'une impulsion d'énergie $U$ et de durée à mi-hauteur $\tau$ par $P = U/\tau$ (Nous avons alors des $\mathrm{J/s}$). Puis, supposons une distribution plate de l'énergie sur un disque de rayon $w_0$. On a une surface $S=\pi w_0^2$, et donc une intensité $I = P/S = U/(\tau S)$. On obtient alors que $137\,\mathrm{\mu J}$ suffisent à ioniser le premier niveau de l'Hélium en première approximation. 

Afin d'affiner cette estimation, supposons une impulsion gaussienne dans les trois dimensions de l'espace. L'expression du mode fondamental d'un champ électromagnétique gaussien dans le plan focal, c'est-à-dire en $z=0$ (Eq.~\ref{eq:HermiteGauss}) s'écrit~:
...
\end{equation}
où $\eta \approx 377\,\mathrm{\Omega}$ est l'impédance caractéristique du vide qui est définie comme $\eta = \frac{1}{\epsilon_0c_0}$.

La distribution temporelle gaussienne de l'impulsion est de largeur à 1/e égale à $\tau$, on alors écrit alors~:
\begin{equation}
    I(r, t)=\frac{E_0^2}{2\eta}\mathrm{e}^{-\frac{2r^2}{w_0^2}}\mathrm{e}^{-\frac{t^2}{\tau^2}}
\end{equation}

La puissance $P(t)$ du faisceau (en W ou J/s) est donnée par~: 
\begin{equation}
...
En utilisant les intégrales gaussiennes connues $\int \mathrm{e}^{-ax^2} dx = \sqrt{\frac{\pi}{a}}$ et $\int x^n\mathrm{e}^{-ax^2} dx = \frac{\Gamma(\frac{n+1}{2})}{2a^{\frac{n+1}{2}}}$, on obtient un facteur $\frac{\sqrt{\pi}}{2}$ par rapport à notre estimation "naïve". Une énergie $U = 137\frac{\sqrt{\pi}}{2}\approx 120\,\mathrm{\mu J}$ est donc suffisante pour ioniser le premier niveau de l'Hélium. 
...
Nous allons à présent nous intéresser au mouvement collectif des électrons dans le plasma en présence d'un champ électromagnétique fort. Cette partie du cours est de la physique des plasmas plutôt que de la physique des lasers, et les démonstrations qui suivent ne font certainement pas partie des choses à savoir pour l'examen, mais ont été inclues pour votre satisfaction intellectuelle : elles apporteront un peu plus de profondeur à votre compréhension de l'interaction lumière-matière pour des champs intenses. On soulignera ici que la physique contemporaine se situe souvent au croisement entre plusieurs domaines. 
...
Cette onde électronique est ce que l'on appelle une onde de sillage lorsqu'elle est excitée par une impulsion laser. Cet nom est donnée par analogie avec l'onde générée dans le sillage d'un bateau (Fig.~\ref{fig:sillage})
...
\subsubsection{Excitation d'une onde plasma par laser}

Nous souhaitons à présent dériver la forme d'une onde de sillage générée par laser en une dimension, et dans le régime linéaire. Nous souhaitons connaître la variation temporelle du potentiel électrostatique $\phi$. Ce dernier dépend linérairement de la séparation spatiale des charges, et en l'absence de forçage on a donc~:


\begin{equation}
    \frac{\partial^2 \phi}{\partial t ^2 }+\omega_p^2\phi=0
    \label{eq:1DlinearNolaser}
\end{equation}

On fait ensuite apparaître un terme de forçage dû au laser. On peut démonter (mais nous ne le ferons pas ici) que la quantité pertinente est $\omega_p^2a(t)^2/2$. $a = \frac{eA}{m_ec}$ est ce que l'on appelle le potentiel vecteur normalisé. On l'obtient en normalisant  par la quantité $\frac{m_ec}{e}$ le module du potentiel vecteur $\textbf{A}$ dont la dérivée par rapport au temps donne le champ électrique~:
\begin{equation}
    E_{laser} = -\frac{\partial \textbf{A}}{\partial t}
\end{equation}
On soulignera ici que le champ éléctrique total est donc la somme de la contribution du laser $E_{laser} = -\frac{\partial \textbf{A}}{\partial t}$ oscillant à la fréquence centrale du laser $\omega_0$ et de celle du plasma $E_{plasma} = -\nabla \phi$ oscillant à la fréquence plasma $\omega_p$.

En ajoutant le terme de forçage dans l'équation~\ref{eq:1DlinearNolaser} on obtient alors l'équation harmonique forcée~:
\begin{equation}
    \frac{\partial^2 \phi}{\partial t ^2 }+\omega_p^2\phi=\frac{\omega_p^2a^2}{2}
    \label{eq:1DlinearWithLaser}
\end{equation}

En résolvant numériquement cette équation pour une impulsion laser de la forme $a(t) = a_0\mathrm{e}^{-t/\tau}$ on a alors une oscillation du potentiel et donc du champ qui en dérive, représentés sur la Figure~\ref{fig:1D_pot_field_lin}.


\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=11cm]{pictures/1D_Field_and_Potential_linear.png}
\end{center}
\caption{Potentiel $\phi$ et champ électrique $E$ de l'onde de sillage dans le régime linéaire}
\label{fig:1D_pot_field_lin}
\end{figure}

En raffinant le modèle, on peut montrer (vous trouverez une démonstration par exemple dans la référence~\cite{FaureHDR}) qu'en une dimension et dans le régime non-linéaire, le potentiel est régi par l'équation suivante~:
\begin{equation}
    \frac{\partial^2 \phi}{\partial \zeta ^2 }\approx\frac{k_p^2}{2}\left(\frac{1+a^2/2}{(1+\phi)^2}-1\right)
    \label{eq:1Dnonlinear}
\end{equation}
L'intensité laser commence à générer des phénomènes relativistes autour de $a>1$. Pour notre exemple, nous allons prendre $a=3$. En résolvant l'équation~\ref{eq:1Dnonlinear} numériquement, on peut alors tracer le potentiel électrostatique $\phi$ et le champ électrique $E$ qui en dérive en fonction de notre variable d'espace-temps $\zeta$ qui se déplace à la vitesse de groupe de l'impulsion laser (voir Fig.~\ref{fig:1D_pot_field}). 
...
\end{center}
\caption{Potentiel $\phi$ et champ électrique $E$ de l'onde de sillage dans le régime non-linéaire}
...
La mécanique Hamiltonienne est la manière la plus élégante d'étudier le mouvement des électrons dans l'onde plasma. Comme la mécanique Newtonienne, elle permet d'établir les équations du mouvement d'un système soumis à un ensemble de forces mais repose sur la conservation de l'énergie au lieu de la conservation de l'impulsion. Dans le cas de notre tranche d'électrons perturbés, on a une énergie cinétique $\frac{p_x^2}{2m_e}$ et une énergie potentielle électrostatique $\Phi_E$ que l'on peut déduire du champ électrique établi dans la section précédente (équation~\ref{eq:chp_E}). On sait tout d'abord que la force électrostatique est conservative, le champ est donc le gradient du potentiel. De plus, l'énergie électrostatique est le produit de la charge et du potentiel, on a donc~:

\begin{equation}
    \textbf{E}=-\nabla V_E \quad \mathrm{et} \quad \Phi_E=-eV_E
\end{equation}

La variable $x$ sur laquelle on dérive le potentiel représente la position de la tranche d'électrons. On déduit $\Phi_E=\frac{e^2n_0}{\epsilon_0}x^2$, ce qui nous permet d'écrire l'Hamiltonien du système~:
\begin{align}
    H &= E_K + \Phi_E \\
...
\end{equation}
où $p$ est la quantité de mouvement et $q$ la position. En appliquant ces relations à l'Hamiltonien~\ref{eq:hamiltonien}, on obtient alors le système d'équations~:
\begin{eqnarray}
...
\end{eqnarray}
On résoud ces équations avec la condition initiale $x(0)=x_0$ et $p(0)=0$. En utilisant $\omega_0^2=\frac{n_0e^2}{m_e\epsilon_0}$ a alors le couple d'équations paramétriques suivant~:
\begin{eqnarray}
       x(t)&=&x_0\cos{(\omega_p t + \varphi)} \\
       p(t)&=&-m_ex_0\omega_0\sin{(\omega_p t + \varphi)}
\end{eqnarray}
En écrivant $x^2(t)+\frac{p^2(t)}{m_e\omega_p}=x_0^2\cos{\omega_pt}+x_0^2\sin{\omega_pt}=x_0^2$, on peut alors facilement représenter $\frac{p^2(t)}{m\omega_p}$ en fonction de $x$, voir Figure~\ref{fig:espace_phases}. C'est ce qu'on appelle l'espace des phases. Cet outil nous sera très utile dans l'analyse des trajectoires des électrons dans le plasma. 
...
\end{center}
\caption{Espace des phases pour un oscillateur harmonique non-amorti. Les différentes trajectoires correspondent à des conditions initiales différentes.}
...
\noindent\fcolorbox{black}{myGray}{%
    \minipage[t]{\dimexpr1\linewidth-2\fboxsep-2\fboxrule\relax}
        ...
        \begin{center}
        \includegraphics[width=8cm]{pictures/empty_phase_space.png}
        \end{center}
...
\end{equation}
Nous définissons $\beta=v_p/c$ pour obtenir l'Hamiltonien que nous utiliserons pour notre analyse des trajectoires~:
\begin{equation}
    H'=\sqrt{1+u_z^2+u_\perp^2}-\phi(\zeta)-\beta_p u_z
\end{equation}
En une dimension, l'Hamiltonien ne varie pas en fonction de $r_\perp$, c'est-à-dire $\partial H/\partial r_\perp = 0$. On peut en déduire que la somme de la composante transverse de la quantité de mouvement $u_\perp$ et de l'amplitude du laser (purement transverse en 1D) doit être constante~:~ $u_\perp(\zeta) - a(\zeta) = \mathrm{cste}$ (où $a(\zeta)$ est le potentiel vecteur normalisé). 
...
Nous avions calculé la forme de l'onde plasma excitée par le laser (equation\ref{eq:1Dnonlinear}, solution représentée sur la figure~\ref{fig:1D_pot_field}). En reprenant l'oscillation calculée numériquement, nous pouvons tracer les trajectoires des particules dans ce potentiel dans l'espace des phases~$(\gamma, k_p\zeta)$.


Nous observons deux types de trajectoires dans cet espace des phases ainsi qu'une trajectoire limite (en rouge) que l'on appelle la séparatrice, et qui délimite les trajectoires piégées des trajectoires non-piégées. La trajectoire en noir représente les électrons qui étaient au repos ($u_z = 0$) à l'arrivée de l'impulsion laser. Ces électrons oscillent dans l'onde plasma un peu comme une bouée oscille sur les vagues : sa quantité de mouvement longitudinale est centrée autour de zéro (on a tracé $u_z+1$ en échelle logarithmique par souci de lisibilité). Les électrons de faible énergie oscillent eux aussi, mais avec une quantité de mouvement moyenne non-nulle, jusqu'à ce que l'énergie soit suffisante pour que ces électrons soient piégés dans l'onde plasma. Les trajectoires fermées correspondent ainsi à des électrons de vitesse longitudinale suffisamment importante pour ``surfer" sur le champ accélérateur et ainsi suivre l'impulsion laser à la vitesse de groupe. C'est ce phénomène de piégeage dans l'onde plasma qui permet l'accélération de particules par laser. 
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=12cm]{pictures/trajectories.png}
\end{center}
\caption{Trajectoires des électrons dans l'onde de sillage. En noir : trajectoire des électrons à l'arrêt devant l'impulsion laser. En rouge : trajectoire limite, dite séparatrice. En bleu : trajectoires d'électrons piégés dans l'onde plasma. }
\label{fig:trajectories}
\end{figure}

\subsubsection{Une expérience d'accélération laser-plasma au laboratoire d'optique appliquée}

Les années '90 ont connu l'essor des lasers basés sur l'amplification dans des cristaux de saphir dopés au titane (plus connus sous le nom de Titane-Saphir) par amplification par dérive de fréquence - ou Chirped Pulse Amplification (CPA) en anglais-. L'introduction de cette technique, initialement issue du monde de l'électronique radio-fréquence, dans le domaine optique a permis de multiplier par près de trois ordres de grandeur la puissance crête des lasers ! Avant l'arrivée de cette technique, l'amplification d'une impulsion comprimée, donc très intense, était limitée par les effets non-linéaires indésirables dans le milieu amplificateur, et à l'extrême limite par la destruction de ce dernier. En étirant une impulsion courte dans le temps, c'est-à-dire en déphasant les différentes fréquences les unes par rapport aux autres, on diminue l'intensité crête, permettant donc l'amplification d'impulsions plus énergétiques. En pratique, l'étirement de l'impulsion se fait la plupart du temps grâce à des réseaux de diffraction. Après amplification, l'impulsion doit être recomprimée, et le processus inverse est aussi effectué avec des réseaux. Un schéma de ce type de système est présenté sur la Figure~\ref{}

\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=12cm]{pictures/cpa.png}
\end{center}
\caption{CPA}
\label{fig:cpa}
\end{figure}

 Les intensités crêtes obtenues grâce au CPA ont permis d'accéder au régime d'interaction laser-plasma relativiste. Plusieurs groupes dans le monde (le LBNL à Berkeley, l'Imperial College à Londres, et le LOA à Palaiseau) ont ainsi pu, et ce presque simultanément, accélérer des électrons à plusieurs centaines des MeV par interaction entre un laser et un plasma généré dans un gaz. 
Le laser ultra-intense alors utilisé générait des impulsions de longueur d'onde centrale 800$\,$nm, d'une énergie d'1$\,$J et d'une durée de 30$\,$fs à mi-hauteur pouvant être focalisées sur 20$\,\mathrm{\mu}$m à mi-hauteur. Les différents blocs fonctionnels de ce laser (la ``Salle Jaune'' du Laboratoire d'Optique Appliquée) sont donnés sur la Figure~\ref{fig:salleJaune}, et une vue d'ensemble avant compression est présentée Figure~\ref{fig:salleJauneLayout}. Vous aurez l'occasion de voir de vos yeux ce type d'installation laser lors de la visite de labo !

\medskip
\noindent\fcolorbox{black}{myGray}{%
    \minipage[t]{\dimexpr1\linewidth-2\fboxsep-2\fboxrule\relax}
        \textbf{Quels sont l'intensité sur cible et le champ électrique maximum ?}\\
        \textit{On supposera l'impulsion gaussienne dans les trois dimensions de l'espace.}
        \vspace{6cm}
    \endminipage}\hfill
\medskip
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=14cm]{pictures/salleJaune.png}
\end{center}
\caption{Blocs fonctionnels de la ``Salle Jaune'', au Laboratoire d'Optique Appliquée}
\label{fig:salleJaune}
\end{figure}


\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=14cm]{pictures/salleJauneLayout.png}
\end{center}
\caption{Vue schématique du laser de la ``Salle Jaune'', avant compression}
\label{fig:salleJauneLayout}
\end{figure}

En fin de chaîne, après recompression de l'impulsion, le laser est focalisé par un réflecteur parabolique (souvenez-vous du premier cours, c'est la seule surface parfaitement stigmatique...) sur une cible de gaz dans laquelle le plasma est généré, comme présenté schématiquement sur la Figure~\ref{fig:accLaserPlasma}. Ce type de dispositif a permis d'obtenir des impulsions d'électrons de 0.5$\,$nC à 170$\,$MeV, ce qui correspond à un transfert d'énergie du laser au paquet d'électrons de 10\%. Ce n'est qu'en 2012 que la durée de ces paquets d'électrons a été mesurée comme étant de l'ordre de 1.5$\,$fs \cite{lundh} ! 

\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=10cm]{pictures/laserPlasmaAcc.png}
\end{center}
\caption{Vue schématique d'un accélérateur laser-plasma}
\label{fig:accLaserPlasma}
\end{figure}
