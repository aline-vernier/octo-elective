...
\begin{center}
\includegraphics[width=11cm]{pictures/SpherMirr.png}
\end{center}
...
L'approximation paraxiale nous permet d'assimiler $\mathrm{sin}(\theta)\sim\mathrm{tan}(\theta)\sim\theta$, et on a ainsi une relation linéraire entre $\left(r_1, \theta_1\right)$ et $\left(r_1, \theta_2\right)$, positions et angles avant et après passage dans le système optique. Si on définit $r_1'$ et $r_2'$ tels que $\theta_1 \sim (dr/dz)_z_1 = r_1'$ et $\theta_2 \sim (dr/dz)_z_2 = r_2'$. Du fait de la linéarité du système optique il existe des coefficients $A, B, C$ et $D$ tels que~: 
...
qui est une équation intégrale de Fredholm du second type admettant des solutions $ \tilde E_{lm}(x,y,0)$ \textbf{auto-similaires après chaque période en cavité}. Chaque solution définie par un couple d'entiers ${l,m}$ sera notée $ \tilde\sigma_{lm}$.
...
Pour des vitesses relativistes, le gain d'énergie n'est évidemment plus linéairement croissant avec le nombre d'étages, et les longueurs des sections deviennent extrêmement longues (l'exemple du SLAC cité précédemment l'illustre). 
...
La distribution temporelle gaussienne de l'impulsion est de largeur à 1/e$^2$ égale à $\tau$, on alors écrit alors~:
\begin{equation}
...
\end{eqnarray}
On résoud ces équations avec la condition initiale $x(0)=x_0$ et $p(0)=0$. En utilisant $\omega_0^2=\frac{n_0e^2}{\epsilon_0}$ a alors le couple d'équations paramétriques suivant~:
\begin{eqnarray}
